...
**Repo:** `E:\Citadel/HardlinkOrganizer/webapp/frontend`
**Verification command:** `npx tsc --noEmit` from `webapp/frontend/` — target is zero errors
**Do not modify** any files in the Fujin repo at `E:\Citadel/Fujin/`

---

## Context

The Fujin UI kit (`E:\Citadel/Fujin`) has been updated with two new features:

1. **`ThemeMenu` component** — a gear icon button + popover with Light/Dark mode toggle.
   Lives at `E:\Citadel/Fujin/components/ThemeMenu.tsx`, exported from the Fujin index.
...
- Do not modify any file in `E:\Citadel/Fujin/`
